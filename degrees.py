...
def shortest_path(source, target):
    ...
    while not frontier.empty():
        # Remove next node from frontier and add it to the explored set
        ...
        if next_node.state == target:
            ...
            while next_node.parent:
                # Insert node and move to parent
                chain.insert(0, (next_node.action, next_node.state))
                next_node = next_node.parent

            return chain

        # Add unexplored child nodes to the frontier
        for neighbor in neighbors_for_person(next_node.state):
            if neighbor[1] in explored:
                continue

            child_node = Node(neighbor[1], next_node, neighbor[0])
